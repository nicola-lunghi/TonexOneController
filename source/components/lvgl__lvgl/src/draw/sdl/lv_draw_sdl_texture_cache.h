...
#define LV_DRAW_SDL_DEC_DSC_TEXTURE_HEAD "@LVSDLTex"
    ...
    SDL_Texture* lv_draw_sdl_texture_cache_get_with_userdata(
        lv_draw_sdl_ctx_t* ctx, const void* key, size_t key_length, bool* found, void** userdata);

    /**
     * @return Whether the texture has been put in the cache
     */
    bool lv_draw_sdl_texture_cache_put(
        lv_draw_sdl_ctx_t* ctx, const void* key, size_t key_length, SDL_Texture* texture);

    /**
     * @return Whether the texture has been put in the cache
     */
    bool lv_draw_sdl_texture_cache_put_advanced(lv_draw_sdl_ctx_t* ctx, const void* key, size_t key_length,
        SDL_Texture* texture, void* userdata, void userdata_free(void*), lv_draw_sdl_cache_flag_t flags);

    lv_draw_sdl_cache_key_head_img_t* lv_draw_sdl_texture_img_key_create(
        const void* src, int32_t frame_id, size_t* size);
