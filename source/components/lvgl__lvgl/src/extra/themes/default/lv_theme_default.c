...
static void style_init(void)
{
    ...
    lv_style_transition_dsc_init(&trans_delayed, trans_props, lv_anim_path_linear, TRANSITION_TIME, 10, NULL);  // GS 70, NULL);
    ...
    lv_style_set_anim_time(&styles->anim, 50); // GS 200);

    style_init_reset(&styles->anim_fast);
    lv_style_set_anim_time(&styles->anim_fast, 10); // GS 120);
...
static void theme_apply(lv_theme_t * th, lv_obj_t * obj)
{
    ...
    if(lv_obj_check_type(obj, &lv_obj_class)) {
        ...
        //lv_obj_add_style(obj, &styles->transition_delayed, 0);    // GS
        lv_obj_add_style(obj, &styles->pressed, LV_STATE_PRESSED);
        //lv_obj_add_style(obj, &styles->transition_normal, LV_STATE_PRESSED);  // GS
