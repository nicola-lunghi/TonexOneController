...
#include "../misc/lv_area.h"
     ...
     *                  pixel               simple set the size accordingly
     *                  LV_SIZE_CONTENT     set the size to involve all children in the given direction
     *                  lv_pct(x)           to set size in percentage of the parent's content area size (the size
     ...
     * @note            Zero return value means the object is on the left padding of the parent, and not on the left
     * edge.
     * @note            Scrolling of the parent doesn't change the returned value.
     * @note            The returned value is always the distance from the parent even if `obj` is positioned by a
     * layout.
     */
     ...
     * @note            The position of the object is recalculated only on the next redraw. To force coordinate
     * recalculation call `lv_obj_update_layout(obj)`.
     * @note            Zero return value means the object is on the right padding of the parent, and not on the right
     * edge.
     * @note            Scrolling of the parent doesn't change the returned value.
     * @note            The returned value is always the distance from the parent even if `obj` is positioned by a
     * layout.
     ...
     * @note            The position of the object is recalculated only on the next redraw. To force coordinate
     * recalculation call `lv_obj_update_layout(obj)`.
     * @return          the width which still fits into its parent without causing overflow (making the parent
     * scrollable)
     */
     ...
     * @note            The position of the object is recalculated only on the next redraw. To force coordinate
     * recalculation call `lv_obj_update_layout(obj)`.
     * @return          the height which still fits into the parent without causing overflow (making the parent
     * scrollable)
     */
     ...
     * @note            The position of the object is recalculated only on the next redraw. To force coordinate
     * recalculation call `lv_obj_update_layout(obj)`.
     * @param area      the area which still fits into the parent without causing overflow (making the parent
     * scrollable)
     */
