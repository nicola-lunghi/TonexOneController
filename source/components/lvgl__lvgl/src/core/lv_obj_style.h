...
#include <stdint.h>
     ...
     * @example         lv_obj_add_style(btn, &btn_red, LV_STATE_PRESSED); //Overwrite only some colors to red when
     * pressed
     */
     ...
     * @param selector  OR-ed values of states and a part to remove only styles with matching selectors. LV_STATE_ANY
...
#include "lv_obj_style_gen.h"
     ...
     * @param part      the part whose opacity should be get. Non-MAIN parts will consider the `opa` of teh MAIN part
     * too
