...
#define LV_STYLE_PROP_ID_MASK(prop) ((lv_style_prop_t) ((prop) & ~LV_STYLE_PROP_META_MASK))
     ...
     *
     * Props are split into groups of 16. When adding a new prop to a group, ensure it does not overflow into the next
     * one.
     */
    typedef enum
    {
     ...
     * @param user_data any custom data that will be saved in the transition animation and will be available when
     * `path_cb` is called
     * @example
     ...
     */
    static inline lv_style_res_t lv_style_get_prop_inlined(
        const lv_style_t* style, lv_style_prop_t prop, lv_style_value_t* value)
    {
